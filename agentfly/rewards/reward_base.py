...
class BaseReward:
    ...
    # ========== Instance Attributes ==========
    def __init__(self, name: str | None = None, func: Callable | None = None, 
                 env_cls: type[BaseEnv] | None = None, pool_size: int = -1, 
                 env_kwargs: dict | None = None, auto_register: bool = True):
        """
        ...
        if '_is_async_call' not in cls.__dict__:
            user_func_is_async = (self.user_func is not None and inspect.iscoroutinefunction(self.user_func))
    ...
    def _check_function_set(self):
        ...
        if self.user_func is None:
            raise ValueError(f"Reward {self.name} has no function set. For inheritance-based rewards, define a 'call' method.")
    ...
    async def _initialize_envs(cls):
        ...
        if cls.env_cls is not None and not cls._initialized:
            await EnvironmentManager.start(cls.env_cls, size=cls.pool_size, env_kwargs=cls.env_kwargs)
    ...
    async def release_all(cls):
        ...
        await asyncio.gather(*[cls._release_single(env_id, success=True) for env_id in env_ids])
...
def register_reward(reward_name: str, reward_function: BaseReward | type[BaseReward]) -> None:
    """
...
def get_rewards_from_names(reward_names: List[str]) -> List[BaseReward | type[BaseReward]]:
    """
