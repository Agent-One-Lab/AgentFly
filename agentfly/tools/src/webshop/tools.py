...
async def webshop_browser(action: str, value: str, env: WebAgentTextEnv):
    ...
    try:
        if action == "search":
            ...
            return f"Error: Invalid action '{action}'. Must be either 'search' or 'click'"
        return observation
