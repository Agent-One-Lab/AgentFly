...
class Qwen3CoderToolParser(ToolParser):
    ...
    def _parse_xml_function_call(
        ...
        # Add known argument-name aliases to the config so type conversion
        # works and we avoid noisy warnings. Our tools use "cmd" but some
        # prompts/models naturally emit "command"; treat them as synonyms.
        if (function_name == "run_shell_command"
                and isinstance(param_config, dict)
                and "cmd" in param_config
                and "command" not in param_config):
            param_config["command"] = param_config["cmd"]
        ...
        # Normalize known aliases in the parsed arguments so they match the
        # actual tool schema used by AgentFly tools.
        if function_name == "run_shell_command":
            # Map "command" -> "cmd" if the model used the more natural name.
            if "command" in param_dict and "cmd" not in param_dict:
                param_dict["cmd"] = param_dict.pop("command")
