...
async def _run_shell(context: Context, cmd: str) -> str:
    ...
    Timeout is set to 60 seconds to avoid long running commands.
    Args:
    ...
    if not is_acquired:
        ...
        if commit is not None:
            await container.run_cmd("git fetch", workdir=WORKSPACE_DIR)
            await container.run_cmd(f"git checkout {commit}", workdir=WORKSPACE_DIR)
    try:
        raw = await container.run_cmd(cmd, timeout=60, workdir=WORKSPACE_DIR)
