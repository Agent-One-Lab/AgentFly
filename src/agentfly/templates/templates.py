...
class Template:
    ...
    def _render_tool_calls(self, tool_calls: List[Dict]) -> str:
        ...
        for tool_call in tool_calls:
            ...
            tool_calls_str.append(
                self.tool_call_template.format(tool_call=json.dumps(tool_call))
            )
    ...
    def _encode_system_message_default(self, tools=None) -> str:
        logger.debug(
            f"[Template] Encoding system message default for template: {self.name}"
        )
        ...
        if self.system_policy.content_processor is not None:
            system_message = self.system_policy.content_processor(
                self.system_message, tools=tools
            )
        else:
        ...
        if tools is None:
            ...
            if self.system_template_with_tools:
                return self.system_template_with_tools.format(
    ...
    def _encode_user_message_with_tools(self, content, tools: str) -> str:
        ...
        if self.user_template_with_tools:
            user_message = self.user_template_with_tools.format(
    ...
    def _encode_assistant_message(self, content, tool_calls_str=None) -> str:
        ...
        if "{tool_calls}" in self.assistant_template:
            assistant_message = self.assistant_template.format(
    ...
    def _encode_generation_prompt(self) -> str:
        ...
        if "{content}" in self.assistant_template:
            ...
            raise ValueError(
                f"Assistant template {self.assistant_template} does not contain {{content}}"
            )
    ...
    def _encode_standard(
        ...
        **kwargs,
        ...
        prompt, elements, roles = self.render(
    ...
    def _jinja_header_constants(self) -> List[str]:
        ...
        if self.system_policy.content_processor is not None:
            ...
            processed_system_message = self.system_policy.content_processor(
                self.system_message, tools=None
            )  # TODO: tools is not used here, but we need to pass it for consistency
            default_system = self.system_template.format(
        ...
        uses_observations = (
            "{observations}" in self.tool_template if self.tool_template else False
        )
        ...
        if self.tool_template:
            header.append(f"{{% set _tool_template = {self.tool_template!r} %}}")
        else:
        ...
        if self.user_template_with_tools:
            ...
            processed_template = self.user_template_with_tools.replace(
                "{{", "{"
            ).replace("}}", "}")
            header.append(
