...
export HYDRA_FULL_ERROR=1
# Conda path
CONDA_BIN_PATH=/path/to/conda/bin/
...
val_file=${VAL_FILE:-/path/to/val_file}
...
tools=${TOOLS:-[answer]}
reward_name=${REWARD_NAME:-vlm_as_judge_reward}

experiment_name=${EXPERIMENT_NAME:-vlm_as_judge}
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="${experiment_name}-${lr}-${length}-bs${batch_size}-n${num_chains}-kl${kl_loss_type}${kl_coef}-entropy${entropy_coeff}-${max_turns}turns-${adv_estimator}" \
